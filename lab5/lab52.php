<?php
    ...
    $archivosValidos = ["jpg", "jpeg", "gif", "png"];
    $valido = 0;
    define('TAM', 1048576);
    for($i = 0; $i < 4; $i++);
    {
        if ($_FILES["nombre_archivo_cliente"]["type"] !== "image/$archivosValidos[$i]")
        {
            $valido = 0;
        }
        else
        ...
        if($_FILES["nombre_archivo_cliente"]["size"] > TAM)
            {
                echo ("El archivo tiene mas de 1 MB");
            }
            else {
...
if ($valido = 0)
{
    echo "El tipo de archivo no es imagen";
}
        ?>
